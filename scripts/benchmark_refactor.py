#!/usr/bin/env python3
"""Benchmark repeated legacy fitting against one-fit-per-boundary refactoring."""
...
def _legacy_repeated_fit(windows, degree: int) -> int:
    fit_calls = 0
    for x, y in windows:
...
def _optimized_fit(windows, degree: int) -> int:
    for x, y in windows:
        np.polyfit(x, y, degree)
    return len(windows)
...
def main() -> None:
    ...
    legacy_s, legacy_calls = _best_time(
        lambda: _legacy_repeated_fit(windows, config.polynomial_degree), args.repeats
    )
    optimized_s, optimized_calls = _best_time(
        lambda: _optimized_fit(windows, config.polynomial_degree), args.repeats
    )
    ...
    result = {
        ...
        "legacy_best_runtime_s": legacy_s,
        "optimized_best_runtime_s": optimized_s,
        "fit_stage_speedup_x": legacy_s / optimized_s,
